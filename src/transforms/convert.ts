...
export function csvToJSON(text: string, opts: CsvOptions = {}): { result: string; error?: string } {
    try {
        ...
        return { result: text, error: t('Invalid CSV') };
    }
}
...
export function jsonToCSV(text: string, opts: CsvOptions = {}): { result: string; error?: string } {
    try {
        ...
        if (!Array.isArray(data) || data.length === 0) {
            return { result: text, error: t('JSON must be a non-empty array of objects') };
        }
...
export function markdownTableToCSV(text: string): { result: string; error?: string } {
    try {
        ...
        if (lines.length < 2) return { result: text, error: t('Not a markdown table') };
        const rows = lines
        ...
        return { result: text, error: t('Invalid markdown table') };
    }
}
